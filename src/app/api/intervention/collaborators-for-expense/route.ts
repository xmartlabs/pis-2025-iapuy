...
export async function GET(request: NextRequest) {
  try {
    ...
    const interventionId = request.nextUrl.searchParams.get("interventionId");
    if (interventionId === null) {
      throw new Error("No se encontro un id de intervención en la solicitud.");
    }
    const involved =
      await interventionsController.getUsersInvolvedInInterventionForExpense(
        payload,
        interventionId
      );
    return NextResponse.json(involved);
  } catch (error) {
