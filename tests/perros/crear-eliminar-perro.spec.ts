...
test('test', async ({ page }) => {
  ...
  await page.getByRole('menuitem', { name: 'Perfil de Bichote' }).click();
  await expect(page.getByRole('heading', { name: 'Bichote' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Eliminar Perro' })).toBeVisible();
  await page.getByText('JP').click();
  await page.getByRole('menuitem', { name: 'Cerrar sesión' }).click();
  ...
  await page.getByRole('button', { name: 'Iniciar' }).click();
  await page.getByRole('link', { name: 'Perros' }).click();
  await page.getByText('Bichote').click();
